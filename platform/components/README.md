...
helm:
  ...
  values:
    ...
    #! Image overrides: use conditional to only set when mirror is configured
    image:
      #@ if data.values.helm_repositories.oci_registry:
      registry: #@ data.values.helm_repositories.oci_registry
      #@ end
...
- Image registry/repository overrides should be conditional on `helm_repositories.oci_registry` so chart defaults are used when no mirror is configured
- Conditional monitoring: wrap with `#@ if data.values.monitoring.enabled:`
- Chart source resolution is handled automatically: the build uses `resolvedChartRef` (from `platform/overlays/`) when `use_mirror=true`, otherwise falls back to `sourceRepo`
